classdef ForecasterPSO
    ...
    methods
        function this = ForecasterPSO(MimariNo)
        ...
        function this = trainAndCalculatePerformances(this)
            ...
            if this.MimariNo == 1
                layers = [ ...
                    ...
                    bilstmLayer(this.numHiddenUnits)
                    fullyConnectedLayer(this.NumResponses)
                    regressionLayer];
            elseif this.MimariNo == 2
                layers = [ ...
                    sequenceInputLayer(this.FeatureDimension)
                    lstmLayer(this.numHiddenUnits)
                    dropoutLayer(0.5)
                    fullyConnectedLayer(this.NumResponses)
                    regressionLayer];
            end

            this.Options = trainingOptions(this.Optimizer, ...
                'MaxEpochs', this.MaxEpochs, ...
                'GradientThreshold',1, ...
                'MiniBatchSize', 64, ...
                'ValidationPatience', 5, ...
                'InitialLearnRate', this.LearningRate, ...
